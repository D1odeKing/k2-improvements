...
from lib.paths import BINARIES_DIR  # noqa: E402
...
def install_python_requirements() -> bool:
    ...
    result = shell.run_logged(command, logger_name="shaketune")
    if not result.ok:
...
def patch_shaper_calibrate() -> bool:
    target = Path("/usr/share/klipper/klippy/extras/shaper_calibrate.py")
    if not target.exists():
        logger.warning("shaper_calibrate.py not found at %s; skipping patch", target)
        return True

    text = target.read_text()
    old = """        configfile = self.printer.lookup_object('configfile')
...
"""
    if new.strip() in text:
        logger.info("shaper_calibrate already patched; skipping")
        return True
    if old.strip() not in text:
        logger.error("Unable to locate expected config block in shaper_calibrate.py")
        return False

    target.write_text(text.replace(old, new, 1))
    logger.info("Patched shaper_calibrate to handle printer=None")
    return True
...
def install_shaketune() -> bool:
    ...
    if not patch_shaper_calibrate():
        logger.warning("Failed to patch shaper_calibrate.py; continuing")
